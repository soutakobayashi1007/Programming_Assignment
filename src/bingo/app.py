...
class Session:
    ...
    def draw_once(self) -> Optional[int]:
        ...
        if not self.started():
            return None
        if count_bingo(self.card.opened) == 12:
            return None
        n = self.balls.draw()
        if n is None:
            return None
    ...
    def status(self) -> Dict[str, Any]:
        ...
        reach = count_reach(self.card.opened)
        bingo = count_bingo(self.card.opened)
        return {
            "last": self.last,
            "drawn": self.balls.drawn(),
            "remaining": self.balls.remaining(),
            "reach": reach,
            "bingo": bingo,
            "complete": (bingo == 12),
        }
